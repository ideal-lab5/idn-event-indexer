	...
	fn write_db_key(
		...
		match self {
			ChainKey::AuctionIndex(auction_index) => {
				let key = U32Key { key: (*auction_index).into(), block_number, event_index };
				trees.auction_index.insert(key.as_bytes(), &[])?
			},
			ChainKey::CandidateHash(candidate_hash) => {
				let key = Bytes32Key { key: candidate_hash.0, block_number, event_index };
				trees.candidate_hash.insert(key.as_bytes(), &[])?
			},
			ChainKey::ParaId(para_id) => {
				let key = U32Key { key: (*para_id).into(), block_number, event_index };
